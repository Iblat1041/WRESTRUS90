...
async def show_group_menu(message: types.Message) -> None:
    ...
    # Создаем deep link для перехода в личный чат
    deep_link = f"https://t.me/{settings.bot_username}?start=group_menu_{message.chat.id}"
    ...
    response = await message.answer(
        "Для работы с меню бота перейдите в личный чат:",
        reply_markup=keyboard,
    )
    # Задержка перед удалением
    await asyncio.sleep(10)  # Увеличиваем время для пользователя
