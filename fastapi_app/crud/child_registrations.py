...
async def get_all_registrations(
        ...
        offset: int = 0,
        limit: int = 10
        ) -> List[ChildRegistration]:
    """Получить список всех регистраций с пагинацией."""
    result = await session.execute(
        select(ChildRegistration)
        .offset(offset)
        .limit(limit)
        .order_by(ChildRegistration.created_at.desc())
        )
...
async def get_registration_by_id(
    ...
    result = await session.execute(
        select(ChildRegistration)
        .where(ChildRegistration.id == reg_id)
        )
